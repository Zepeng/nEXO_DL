...
class UserModel(XModel):
    def __init__(self, sys_bits, x_int_bits, *args, **kwargs):
        ...
        # Conv block 1: 16 filters, 3x3 kernel, stride 1, ReLU, MaxPool 2x2
        self.conv1 = XBundle(
            core=XConvBN(
                k_int_bits=0, b_int_bits=0, filters=16, kernel_size=3, strides=1,
                act=XActivation(sys_bits=sys_bits, o_int_bits=0, type='relu', slope=0)),
            pool=XPool(
                type='max', pool_size=2, strides=2, padding='valid',
                act=XActivation(sys_bits=sys_bits, o_int_bits=0, type=None))
        )

        # Conv block 2: 16 filters, 3x3 kernel, stride 1, ReLU, MaxPool 2x2
        self.conv2 = XBundle(
            core=XConvBN(
                k_int_bits=0, b_int_bits=0, filters=16, kernel_size=3, strides=1,
                act=XActivation(sys_bits=sys_bits, o_int_bits=0, type='relu', slope=0)),
        ...
        # Conv block 3: 24 filters, 3x3 kernel, stride 1, ReLU, MaxPool 2x2
        self.conv3 = XBundle(
            core=XConvBN(
                k_int_bits=0, b_int_bits=0, filters=24, kernel_size=3, strides=1,
                act=XActivation(sys_bits=sys_bits, o_int_bits=0, type='relu', slope=0)),
            pool=XPool(
                type='max', pool_size=2, strides=2, padding='valid',
                act=XActivation(sys_bits=sys_bits, o_int_bits=0, type=None)),
        ...
        # Dense block 1: 42 neurons, ReLU
        self.dense1 = XBundle(
            core=XDense(
                k_int_bits=0, b_int_bits=0, units=42, use_bias=False,
                act=XActivation(sys_bits=sys_bits, o_int_bits=0, type='relu', slope=0))
        )

        # Dense block 2: 64 neurons, ReLU
        self.dense2 = XBundle(
            core=XDense(
                k_int_bits=0, b_int_bits=0, units=64, use_bias=False,
                act=XActivation(sys_bits=sys_bits, o_int_bits=0, type='relu', slope=0))
        )

        # Output layer: 2 neurons (regression output)
        self.output_dense = XBundle(
            core=XDense(
...
save_model(model, "nexo_cnn.h5")
loaded_model = load_qmodel("nexo_cnn.h5")
